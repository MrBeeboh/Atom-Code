  ...
  async function load() {
    ...
    try {
      ...
      if (smallest) {
        ...
        if (stored.trim()) {
          modelSelectionNotification.set(
            `Previous model unavailable, selected ${smallest}`,
          );
      ...
      models.set([]);
      modelSelectionNotification.set(
        "Cannot connect to LM Studio. Please ensure server is running.",
      );
    } finally {
      loading = false;
    }
...
<div class="flex flex-col gap-1 min-w-0">
  <div class="flex items-center gap-2">
    ...
    >
      <button
        type="button"
        class="flex items-center gap-2 rounded-xl border text-sm px-3 py-1.5 max-w-[420px] focus:ring-2 focus:ring-offset-1 font-semibold min-h-[36px] transition-colors duration-150 ui-model-selector {open
      ...
      >
        {#if $selectedModelId}
          ...
          <img
            src={selIcon}
            alt=""
            class="w-4 h-4 shrink-0 rounded object-contain"
          />
          <span class="truncate font-bold uppercase tracking-tight text-xs"
            >{modelDisplayName($selectedModelId)}</span
          >
          {#if getModelTypeTag($selectedModelId)}
            <span
              ...
              >{getModelTypeTag($selectedModelId)}</span
            >
          {/if}
        ...
        {/if}
        <svg
          class="w-4 h-4 shrink-0 ml-1 transition-transform duration-150 {open
            ? 'rotate-180'
            : ''}"
          fill="none"
          viewBox="0 0 24 24"
      ...
      {#if open}
        <div
          id="model-listbox"
          class="fixed z-[100] rounded-[12px] py-1 overflow-y-auto overflow-x-visible glass-modal"
          style="left: {dropdownPlace.left}px; min-width: {dropdownPlace.width}px; width: max-content; max-width: min(calc(100vw - {dropdownPlace.left +
            24}px), 800px); max-height: {dropdownPlace.maxHeight}px; {dropdownPlace.openUp
            ? 'bottom: ' + dropdownPlace.bottom + 'px; top: auto;'
            : 'top: ' + dropdownPlace.top + 'px;'}"
          role="listbox"
        >
          {#if loading}
            <div
    ...
    {#if $selectedModelId && getQuantization($selectedModelId)}
      <span
        class="font-mono text-[10px] px-1.5 py-0.5 rounded bg-zinc-200 dark:bg-zinc-600 text-zinc-600 dark:text-zinc-400 shrink-0"
        title="Quantization">{getQuantization($selectedModelId)}</span
      >
    {/if}
  </div>
  {#if $modelSelectionNotification}
    <p
      ...
      style="color: var(--ui-text-secondary);"
      title={$modelSelectionNotification}
    >
      {$modelSelectionNotification}
    </p>
  {/if}
