  ...
  >
    ...
    <div class="flex-1 overflow-y-auto px-6 py-4 space-y-3">
      ...
      >
        ...
        <div class="px-4 py-3 space-y-3">
          <p class="text-xs text-zinc-500 dark:text-zinc-400">
            Additional instructions for ATOM Code. These are appended after the
            built-in coding assistant instructions. Use this to set your
            preferred coding style, project-specific rules, or personal
            preferences.
          </p>
          <textarea
            bind:value={$userSystemPrompt}
            placeholder="Example: &quot;Always use TypeScript. Prefer functional components. Never use var, always const or let.&quot;"
            class="w-full h-32 rounded border-zinc-300 px-3 py-2 text-zinc-900 dark:text-zinc-100 text-sm font-mono placeholder:text-zinc-400 glass-modal resize-none"
          ></textarea>
          <div class="flex justify-end">
            <button
              type="button"
              class="text-xs text-red-500 hover:text-red-600"
              onclick={() => userSystemPrompt.set("")}
              >Reset AI behavior to default</button
            >
          </div>
        </div>
      </details>
