<script>
  let { nodes = [], expandedDirs = new Set(), pinnedSet = new Set(), openFilePath = null, level = 0, onToggleDir, onPin, onUnpin, onOpenFile, onCopyPath, onContextMenu } = $props();
...
{#each nodes || [] as node (node.path)}
  ...
  {:else}
    <button
      ...
      style="color: var(--ui-text-primary); margin-left: {level * 8 + 12}px; {isOpen(node.path) ? 'background: color-mix(in srgb, var(--ui-accent) 14%, transparent); border-left: 2px solid var(--ui-accent);' : ''}"
      onclick={() => onOpenFile?.(node.path)}
      oncontextmenu={(e) => onContextMenu?.(e, node)}
    >
      {#if isOpen(node.path)}
        <span class="shrink-0 w-1.5 h-1.5 rounded-full" style="background: var(--ui-accent);" title="Open in editor"></span>
      {/if}
